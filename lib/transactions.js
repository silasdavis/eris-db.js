...
const createArgs = exports.createArgs = (account, value) => [2, {
  address: account,
  permission: 4,
  value: value
}]

const createInput = exports.createInput = (user, sequence, publicKey) => {
  const input = {
    address: user,
    amount: 1,
    sequence: sequence + 1
...
/*
Eris DB uses its own custom JSON.stringify function that doesn't generate
...
*/
exports.writeSignBytes = (chainId, input, type, args) =>
  '{"chain_id":' + canonicalJson(chainId) + ',"tx":[' + canonicalJson(type) +
  ',{"args":"' + canonicalJson(args) + '","input":' + canonicalJson(input) +
  '}]}'

exports.createSetCallPermission = (input, type, args, signature) => [
  type,
  {
    input: R.assoc('signature', [1, signature], input),
    args: args
  }
]
