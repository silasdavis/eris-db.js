...
exports.input = (user, sequence, publicKey) => {
  const input = {
    address: user,
    amount: 1,
    sequence: sequence + 1
  ...
  return sequence === 0
    ? R.assoc('pub_key', publicKey, input)
    : input
}

function createTransaction (type, input, properties, signature) {
  return [
    type,
    R.merge({input: R.assoc('signature', [1, signature], input)}, properties)
  ]
}
...
exports.Permissions = {
  ...
  args: (account, permission, value) => [2, {
    address: account,
    permission: permission,
    value: value
  }],

  signBytes: function (chainId, input, args) {
    return '{"chain_id":' + canonicalJson(chainId) +
    ',"tx":[' + this.type + ',{"args":"' + canonicalJson(args) + '","input":' +
    canonicalJson(input) + '}]}'
  },

  transaction: function (input, args, signature) {
    return createTransaction(this.type, input, {args: args}, signature)
  }
}
...
exports.Call = {
  ...
  signBytes: function (chainId, input, address, data) {
    return '{"chain_id":' +
    canonicalJson(chainId) + ',"tx":[' + this.type + ',{"address":' + canonicalJson(address) +
    ',"data":' + canonicalJson(data) + ',"fee":null,"gas_limit":null,"input":' +
    canonicalJson(input) + '}]}'
  },

  transaction: function (input, address, data, signature) {
    return createTransaction(this.type, input, {
      address: address,
      data: data,
      fee: null,
      gas_limit: null
