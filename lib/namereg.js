 ...
 * with the client.
 *
 * Note: This requires a private key to be sent to eris-db, and should only be done when javascript and
 * eris-db server is running on the same machine (or in the same, private network).
 *
...
NameReg.prototype.setEntry = function (privKey, name, data, numBlocks, callback) {
    ...
    this._events.subNewBlocks(function(error){
        ...
        that.getEntry(name, function(error, data){
            if(error){
                callback(error);
            } else {
                callback(null, data);
    ...
    this._unsafe.transactNameReg(privKey, name, data, this.calculateCost(numBlocks, data), 0, null, function(error){
        if(error) {
            callback(error);
        }
    });
