...
Unsafe.prototype.transactAndHold = function (privKey, address, data, gasLimit, fee, context, callback) {
    ...
    var param = rpc.transactParam(privKey, address, data, gasLimit, fee);
    var fixedCallback = function(error, data2){ // NOTE: is this safe? what if there are more arguments?
	    	if (data2 && data2.constructor == Array){
			data2 = data2[1] // tendermint rpc results is prefixed with a type byte
		}
