        ...
        return erisDb.keyServer.publicKeyFor({address: user})
          .then(function (publicKey) {
            return Promise.fromCallback(function (callback) {
              ...
              const type = 32
              const input = transactions.createInput(user, sequence, publicKey)
              const args = transactions.createArgs(address, value)

              const signBytes = transactions.writeSignBytes(erisDb.name, input,
                type, args)

              return erisDb.keyServer.sign(signBytes, {address: user})
                .then(function (signature) {
                  const transaction = transactions.createSetCallPermission(
                    input, type, args, signature.toString('hex')
                  )

                  return Promise.fromCallback(
                    callback => erisDb.txs().broadcastTx(transaction, callback)
                  )
                })
            })
          })
