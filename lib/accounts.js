        ...
        return erisDb.keyServer.publicKeyFor({address: user})
          .then(function (publicKey) {
            return Promise.fromCallback(function (callback) {
              ...
              const input = transactions.input(user, sequence, publicKey)
              const args = transactions.Permissions.args(address, 4, value)

              const signBytes = transactions.Permissions.signBytes(erisDb.name,
                input, args)

              return erisDb.keyServer.sign(signBytes, {address: user})
                .then(function (signature) {
                  const transaction = transactions.Permissions.transaction(
                    input, args, signature.toString('hex')
                  )
