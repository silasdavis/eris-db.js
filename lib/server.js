...
const transportWrapper = (transport, requestsObserver, responsesObserver) =>
  R.pipe(
    I.map(R.pipe(
      convertIdToString,
      addErisDbNamespace,
    ...
    I.split(requestsObserver),
    transport,
    I.split(responsesObserver),
...
module.exports = (serverUrl, options = {}) => {
  const client = jsonRpc.client({
    ...
    transport: transportWrapper(
      ...
      options.requestsObserver || I.sink,
      options.responsesObserver || I.sink
    )
  })
